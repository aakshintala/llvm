...
#define DEBUG_TYPE "tgsistructcfg"
...
namespace llvm {
  void initializeTGSICFGStructurizerPass(PassRegistry&);
}
...
namespace {
class TGSICFGStructurizer : public MachineFunctionPass {
public:
  ...
  TGSICFGStructurizer() :
    ...
    initializeTGSICFGStructurizerPass(*PassRegistry::getPassRegistry());
  }

  StringRef getPassName() const override {
    return "TGSI Control Flow Graph structurizer Pass";
  }
...
protected:
  ...
  /// because the TGSI instruction is not recognized as terminator fix this
...
int TGSICFGStructurizer::getSCCNum(MachineBasicBlock *MBB) const {
...
MachineBasicBlock *TGSICFGStructurizer::getLoopLandInfo(MachineLoop *LoopRep)
    const {
...
bool TGSICFGStructurizer::hasBackEdge(MachineBasicBlock *MBB) const {
...
bool TGSICFGStructurizer::isRetiredBlock(MachineBasicBlock *MBB) const {
  MBBInfoMap::const_iterator It = BlockInfoMap.find(MBB);
  if (It == BlockInfoMap.end())
    return false;
...
bool TGSICFGStructurizer::isActiveLoophead(MachineBasicBlock *MBB) const {
  MachineLoop *LoopRep = MLI->getLoopFor(MBB);
...
TGSICFGStructurizer::PathToKind TGSICFGStructurizer::singlePathTo(
    MachineBasicBlock *SrcMBB, MachineBasicBlock *DstMBB,
...
int TGSICFGStructurizer::countActiveBlock(MBBVector::const_iterator It,
    MBBVector::const_iterator E) const {
  int Count = 0;
  while (It != E) {
...
bool TGSICFGStructurizer::needMigrateBlock(MachineBasicBlock *MBB) const {
...
MachineInstr *TGSICFGStructurizer::insertInstrBefore(MachineBasicBlock *MBB,
                                                       int NewOpcode,
                                                       const DebugLoc &DL) {
  MachineInstr *MI =
      MBB->getParent()->CreateMachineInstr(TII->get(NewOpcode), DL);
...
MachineInstr *TGSICFGStructurizer::insertInstrBefore(
    MachineBasicBlock::iterator I, int NewOpcode) {
...
MachineBasicBlock *
TGSICFGStructurizer::getFalseBranch(MachineBasicBlock *MBB,
    MachineInstr *MI) {
...
bool TGSICFGStructurizer::isCondBranch(MachineInstr *MI) {
   switch (MI->getOpcode()) {
      //case TGSI::JUMP_COND:
      case TGSI::P_BRCOND: 
         return true;
...
bool TGSICFGStructurizer::isUncondBranch(MachineInstr *MI) {
  switch (MI->getOpcode()) {
  case TGSI::BRA:
    return true;
  }
...
DebugLoc TGSICFGStructurizer::getLastDebugLocInBB(MachineBasicBlock *MBB) {
...
MachineInstr *TGSICFGStructurizer::getNormalBlockBranchInstr(
    MachineBasicBlock *MBB) {
...
MachineInstr *TGSICFGStructurizer::getReturnInstr(MachineBasicBlock *MBB) {
  MachineBasicBlock::reverse_iterator It = MBB->rbegin();
  if (It != MBB->rend()) {
    MachineInstr *instr = &(*It);
...
bool TGSICFGStructurizer::isReturnBlock(MachineBasicBlock *MBB) {
...
void TGSICFGStructurizer::cloneSuccessorList(MachineBasicBlock *DstMBB,
    MachineBasicBlock *SrcMBB) {
...
MachineBasicBlock *TGSICFGStructurizer::clone(MachineBasicBlock *MBB) {
...
void TGSICFGStructurizer::replaceInstrUseOfBlockWith(
...
void TGSICFGStructurizer::wrapup(MachineBasicBlock *MBB) {
...
bool TGSICFGStructurizer::prepare() {
  bool Changed = false;

  //FIXME: if not reducible flow graph, make it so ???

  DEBUG(dbgs() << "TGSICFGStructurizer::prepare\n";);
...
bool TGSICFGStructurizer::run() {

  //Assume reducible CFG...
  DEBUG(dbgs() << "TGSICFGStructurizer::run\n");
...
void TGSICFGStructurizer::orderBlocks(MachineFunction *MF) {
  int SccNum = 0;
  MachineBasicBlock *MBB;
...
int TGSICFGStructurizer::patternMatch(MachineBasicBlock *MBB) {
  int NumMatch = 0;
  int CurMatch;
...
int TGSICFGStructurizer::patternMatchGroup(MachineBasicBlock *MBB) {
  int NumMatch = 0;
...
int TGSICFGStructurizer::serialPatternMatch(MachineBasicBlock *MBB) {
  if (MBB->succ_size() != 1)
    return 0;
...
int TGSICFGStructurizer::ifPatternMatch(MachineBasicBlock *MBB) {
  //two edges
  if (MBB->succ_size() != 2)
    return 0;
  if (hasBackEdge(MBB))
...
int TGSICFGStructurizer::loopendPatternMatch() {
...
int TGSICFGStructurizer::mergeLoop(MachineLoop *LoopRep) {
  MachineBasicBlock *LoopHeader = LoopRep->getHeader();
  MBBVector ExitingMBBs;
  LoopRep->getExitingBlocks(ExitingMBBs);
...
bool TGSICFGStructurizer::isSameloopDetachedContbreak(
...
int TGSICFGStructurizer::handleJumpintoIf(MachineBasicBlock *HeadMBB,
    MachineBasicBlock *TrueMBB, MachineBasicBlock *FalseMBB) {
...
int TGSICFGStructurizer::handleJumpintoIfImp(MachineBasicBlock *HeadMBB,
    MachineBasicBlock *TrueMBB, MachineBasicBlock *FalseMBB) {
  int Num = 0;
...
void TGSICFGStructurizer::showImproveSimpleJumpintoIf(
...
int TGSICFGStructurizer::improveSimpleJumpintoIf(MachineBasicBlock *HeadMBB,
    MachineBasicBlock *TrueMBB, MachineBasicBlock *FalseMBB,
    MachineBasicBlock **LandMBBPtr) {
  ...
  //insert TGSI::ENDIF to avoid special case "input landBlk == NULL"
  MachineBasicBlock::iterator I = insertInstrBefore(LandBlk, TGSI::ENDIF);
...
void TGSICFGStructurizer::mergeSerialBlock(MachineBasicBlock *DstMBB,
    MachineBasicBlock *SrcMBB) {
  DEBUG(
...
void TGSICFGStructurizer::mergeLooplandBlock(MachineBasicBlock *DstBlk,
    MachineBasicBlock *LandMBB) {
...
void TGSICFGStructurizer::mergeLoopbreakBlock(MachineBasicBlock *ExitingMBB,
    MachineBasicBlock *LandMBB) {
...
void TGSICFGStructurizer::settleLoopcontBlock(MachineBasicBlock *ContingMBB,
    MachineBasicBlock *ContMBB) {
...
int TGSICFGStructurizer::cloneOnSideEntryTo(MachineBasicBlock *PreMBB,
    MachineBasicBlock *SrcMBB, MachineBasicBlock *DstMBB) {
  int Cloned = 0;
...
MachineBasicBlock *
TGSICFGStructurizer::cloneBlockForPredecessor(MachineBasicBlock *MBB,
    MachineBasicBlock *PredMBB) {
...
void TGSICFGStructurizer::migrateInstruction(MachineBasicBlock *SrcMBB,
    MachineBasicBlock *DstMBB, MachineBasicBlock::iterator I) {
  MachineBasicBlock::iterator SpliceEnd;
  //look for the input branchinstr, not the TGSI branchinstr
...
MachineBasicBlock *
TGSICFGStructurizer::normalizeInfiniteLoopExit(MachineLoop* LoopRep) {
  MachineBasicBlock *LoopHeader = LoopRep->getHeader();
...
void TGSICFGStructurizer::removeUnconditionalBranch(MachineBasicBlock *MBB) {
...
void TGSICFGStructurizer::removeRedundantConditionalBranch(
    MachineBasicBlock *MBB) {
  if (MBB->succ_size() != 2)
    return;
...
void TGSICFGStructurizer::addDummyExitBlock(
...
void TGSICFGStructurizer::removeSuccessor(MachineBasicBlock *MBB) {
  while (MBB->succ_size())
    MBB->removeSuccessor(*MBB->succ_begin());
}

void TGSICFGStructurizer::recordSccnum(MachineBasicBlock *MBB,
    int SccNum) {
...
void TGSICFGStructurizer::retireBlock(MachineBasicBlock *MBB) {
  DEBUG(
...
char TGSICFGStructurizer::ID = 0;

} // end anonymous namespace


INITIALIZE_PASS_BEGIN(TGSICFGStructurizer, "tgsistructurizer",
                      "TGSI CFG Structurizer", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_DEPENDENCY(MachinePostDominatorTree)
INITIALIZE_PASS_DEPENDENCY(MachineLoopInfo)
INITIALIZE_PASS_END(TGSICFGStructurizer, "tgsistructurizer",
                      "TGSI CFG Structurizer", false, false)

FunctionPass *llvm::createTGSICFGStructurizerPass() {
  return new TGSICFGStructurizer();
}
